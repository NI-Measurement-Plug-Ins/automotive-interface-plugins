...
The Packets Received field also displays the number of packets received on the bus
The Displaying Icon is displayed in status window when new data is getting pulled from the bus
If there is no new data, "Waiting For Data' is displayed instead
 ![image](<img/09.png>)
...
Set the interface to match that of the interface specified in the Plugin UI
All the fields can be manipulated so the user can test how packets are recieved and displayed in the UI
